...
├── Host.py                 ← Host entry point (generated by VIS new)
...
| Method | Description |
|--------|-------------|
| `host.open(screen_name, stay_open=False)` | Unified navigation. Tabbed screens open as tabs; standalone screens are spawned as subprocesses. |
| `host.tick_fps()` | Call once per update loop iteration to maintain `host.fps`. |
| `host.quit_host()` | Fully shuts down the Host — stops the tray icon, stops the IPC server, clears the singleton, and destroys the window. Safe to call from any thread. Also wired to the tray **Quit** item. |
...
| `screen_name` | `str` | Name of the screen to open, or `"__VIS_QUIT__"` to stop the Host |

Returns `True` if the message was delivered, `False` if no Host port file was found or the connection failed.

**How it works:** The Host writes its TCP port number to `%TEMP%/<ProjectTitle>_vis_host.port` on startup and deletes it on shutdown. `send_to_host()` reads that file, connects to `127.0.0.1:<port>`, and sends the screen name as UTF-8 text. The Host receives the message on its IPC listener thread and schedules `host.open(name)` on the main thread via `self.after(0, ...)`.

This is the mechanism used by `VIS stop` and by `Screen.load()` when a Host is detected.
