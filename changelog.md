...
- **Native `image` / `compound` in rounded mode (#188) — corner-tile rounding (`RoundedLeaf`).** The earlier rounded-leaf approach painted the fill into the widget's single Tk image slot, which forced `compound="center"` and left no slot for a caller's own `image=` — so an icon+text button drew the text *over* the icon instead of beside it. The leaf widgets (`vLabel`/`vButton`) now round the **other** way, via a new `RoundedLeaf` mixin (`VIStk/Widgets/_vLeaf.py`): the widget stays an ordinary **solid-background** Label/Button and the corners are rounded by four small **overlay tiles** — each an exact crop of the very same `rounded_pil_image` the containers paint, so the corners look pixel-identical — plus four thin edge strips that carry the `outline` along the straight edges. The image slot is therefore **free**, so `image` / `text` / `compound` / `anchor` all behave **exactly like native tkinter** (icon-left, icon-top, … lay out with no overlap), and the widget sizes to its native content like a normal Label/Button. Hover (`active_fill`) and `disabled_fill` now recolour the widget's real `bg` (the tiles follow on the forced repaint); the tiles forward clicks and hover so the rounded corners stay clickable. Removes the whole glyph machinery (`_glyph_to_pil` / `_composite_glyph` / the `image=` interception in `configure`). Caveat: a tile sits over each extreme corner, so content pushed fully into a corner (e.g. `anchor="nw"` text) can be clipped by ~`radius` px — a non-issue for normal centred / `compound` content, which is inset by `padx`/`pady`. Containers (`vFrame`/`vLabelFrame`) round via `RoundedContainer` (a lowered background label) and are unchanged.
...
- `class vLabel(RoundedLeaf, vWidget, Label)` — inherits `bg`/`fg`/`font`. Rounded mode keeps it an ordinary solid-background Label and rounds the corners with overlay tiles (`RoundedLeaf`), so `image` / `text` / `compound` / `anchor` behave exactly like a native `Label` (icon + text lay out via `compound` with no overlap). Drop-in for `tkinter.Label` at `radius=0`.

**`vButton`** — `VIStk/Widgets/_vButton.py`

- `class vButton(RoundedLeaf, vWidget, Button)` — inherits `bg`/`fg`/`font`; `command`/`invoke()` and all native button options pass through, including native `image`/`compound` (icon beside/above text). Rounded mode flattens the relief (`relief="flat"`, `overrelief="flat"`), shows a hand cursor, and rounds via `RoundedLeaf` tiles. An optional `active_fill` recolours the button on hover (`<Enter>`/`<Leave>`) and `disabled_fill` + `configure(state="disabled")` greys it, swaps the cursor, and gates the click (native `state` already blocks `invoke`) — the corner tiles follow the recolour and forward clicks/hover so the corners stay live. Drop-in for `tkinter.Button` at `radius=0`.
