...
- `class vFrame(vWidget, LayoutFrame)` — keeps the `.Layout` helper; inherits `background` only (Frames have no fg/font). The frame's `bg` is the **fill**; rounded mode draws one anti-aliased rounded rectangle (fill + optional `outline`) onto a *lowered* background `Label`. Two layers keep the corners clean:
